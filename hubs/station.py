...
class StationState(Enum):
    ...
    # Inner signal -- Passengers are boarding - train is at the station (wait for timer)
    BOARD = 4
...
class Station:
    ...
    def step(self, out):
        ...
        if out == ArriveEvent.LEAVE:  # If the train leaves the station
            ...
            if self.near:
                # If there is a train near the station, signal is GREEN
                outersignal = StationState.GREEN
                self.near = False
            else:
                outersignal = StationState.RED  # Signal is red
...
class StationInterface:
    ...
    def is_waiting(self, port):
        # If a train arrives at the station, it waits
    ...
    def step(self):
        ...
        # If the station signal is green
        if self.action_taken[0] == StationState.GREEN:
        ...
        if self.occupied and self.station.occupied == False:  # If the station is occupied
            ...
            # If the station is not otherwise occupied, then nothing is leaving the station
            self.ports[1] = None
    ...
    def is_consumed(self, port):
        if port == 0:  # The station becomes occupied
            ...
            return self.ports[0] == ArriveEvent.ENTER or self.internal == ArriveEvent.ENTER
        else:
            return False
...
def testwait(stationInterface):
    while not stationInterface.is_produced(1):
        ...
        print("Port 0: Consumed ({}), Produced ({})".format(
            stationInterface.is_consumed(0), stationInterface.is_produced(0)))
        print("Port 1: Consumed ({}), Produced ({})".format(
            stationInterface.is_consumed(1), stationInterface.is_produced(1)))
        print("Port 0: Waiting ({}), Port 1: Waiting ({})".format(
            stationInterface.is_awaiting(0), stationInterface.is_awaiting(1)))
        print()
...
def test():
    ...
    interface.step()
    print("Port 0: Consumed ({}), Produced ({})".format(
        interface.is_consumed(0), interface.is_produced(0)))
    print("Port 1: Consumed ({}), Produced ({})".format(
        interface.is_consumed(1), interface.is_produced(1)))
    print("Port 0: Waiting ({}), Port 1: Waiting ({})".format(
        interface.is_awaiting(0), interface.is_awaiting(1)))
    print()
    interface.is_waiting(0)
    testwait(interface)
    interface.step()
